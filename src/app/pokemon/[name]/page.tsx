...
import { notFound } from "next/navigation"; // ✅ Import obligatorio
...
async function getPokemon(name: string): Promise<Pokemon> {
  ...
  if (!res.ok) {
    notFound(); // ✅ Mostrar directamente la plantilla 404
  }
    ...
    <div className="p-8">
      <div className="max-w-4xl mx-auto bg-white rounded-2xl shadow-2xl overflow-hidden">
        <div className={`bg-linear-to-r from-black ${typeColors[pokemon.types[0].type.name]} p-8`}>
          <h1 className="text-5xl font-bold text-white capitalize text-center">{pokemon.name}</h1>
